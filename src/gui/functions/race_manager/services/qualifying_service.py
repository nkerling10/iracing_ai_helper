...
class QualifyingService:
    ...
    def _sort_owner_points(race_manager, cars_drivers_left):
        ...
        for car_driver in cars_drivers_left:
            try:
                points_rank = [
                    race_manager.season_data.owner_points.index(points_entry)
                    for points_entry in race_manager.season_data.owner_points
                    if car_driver[0] == points_entry[0]
                ][0]
    ...
    def _set_field_size(cls, race_manager) -> None:
        ...
        race_manager.season_data.pole_winner = [
            driver["UserName"]
            for driver in race_manager.ir["DriverInfo"]["Drivers"]
            if driver["CarIdx"] == qualifying_results[0]["CarIdx"]
        ][0]
        logging.info(
        ...
        logger.debug(
            f"Setting positions 1-{race_manager.race_weekend.race_settings.field_size - 5}"
        )
        for position in range(
            0, race_manager.race_weekend.race_settings.field_size - 5
        ):
            cars_in_race.append(
                [
                    driver["CarNumber"]
        ...
        non_locked_in_qual_cars = qualifying_results[
            race_manager.race_weekend.race_settings.field_size - 5 :
        ]
        cars_drivers_left = []
        for position in non_locked_in_qual_cars:
            cars_drivers_left.append(
                [
                    (driver["CarNumber"], driver["UserName"])
                    for driver in race_manager.ir["DriverInfo"]["Drivers"]
                    if driver["CarIdx"] == position["CarIdx"]
                ][0]
            )
        ...
        logger.debug(
            f"Setting positions {race_manager.race_weekend.race_settings.field_size - 4}-{race_manager.race_weekend.race_settings.field_size - 1}"
        )

        if points_sorting_ranked:
            for car in range(0, 3):
                logger.debug(
                    f"{points_sorting_ranked[car][0]} car made the field on owners points provisional"
                )
                cars_in_race.append(points_sorting_ranked[car][0])
                match = [
                    pair
                    for pair in cars_drivers_left
                    if pair[0] == points_sorting_ranked[car][0]
                ][0]
                cars_drivers_left.remove(match)
        else:
            logger.warning(
                "No owner points provisional detected, skipping to winner's provisional"
            )
        ...
        for car_driver in cars_drivers_left:
            try:
                driver_info = [
                    driver
                    for driver in race_manager.season_data.drivers_list
                    if driver[0] == car_driver[1]
                ][0]
            ...
            if driver_info[7]:
                ...
                try:
                    match = [
                        driver
                        for driver in race_manager.season_data.current_season_winners
                        if car_driver[1] == driver[0]
                    ][0]
                    last_current_season_winners.append(car_driver)
                except IndexError:
                    continue
        ...
        if len(series_past_champions) == 1:
            logger.debug(
                f"{series_past_champions[0][1]} made the field on past champion provisional"
            )
            ...
            logger.warning(
                f"Number of series champions is more than one. Only adding the first found ({series_past_champions[0][1]}) into the list"
            )
        ...
        # No past champions, check for previous/current season race winners
            ...
            if len(last_current_season_winners) == 1:
                logger.debug(
                    f"{last_current_season_winners[0][1]} made the field on previous/current season winner's provisional"
                )
                ...
                logger.warning(
                    f"Number of race winners is more than one. Only adding the first found ({last_current_season_winners[0][1]}) into the list"
                )
                logger.critical("This tie-breaking feature needs to be implemented!")
                cars_in_race.append(last_current_season_winners[0][0])
                cars_drivers_left.remove(last_current_season_winners[0])
            # As a last effort, take the next highest in owners points
                ...
                points_sorting_ranked_final = cls._sort_owner_points(
                    race_manager, cars_drivers_left
                )
                if points_sorting_ranked_final:
                    logger.debug(
                        f"{points_sorting_ranked_final[0][0]} made the final spot on owners points provisional"
                    )
                    cars_in_race.append(points_sorting_ranked_final[0][0])
                else:
                    logger.debug(
                        "No owner points provisional detected, skipping to speed entries"
                    )

        while len(cars_in_race) < race_manager.race_weekend.race_settings.field_size:
            logger.debug(
                f"{cars_drivers_left[0][1]} has made the race on speed after provisionals were applied"
            )
        ...
        for car in cars_drivers_left:
            ...
            race_manager.send_iracing_command(f"!dq {car[0]} #{car[0]} missed the race")

    ...
    def calculate_stage_points(race_manager):
        for stage in race_manager.race_weekend.stage_results[0:1]:
            for driver_name in stage.stage_results:
                ...
                driver_obj.stage_wins += 1 if stage.stage_results.index(driver_name) == 0 else 0
    ...
    def calculate_race_points(race_manager):
        for position in race_manager.race_weekend.stage_results[2].stage_results:
            ...
            if not position["ReasonOutStr"] == "DQInv":
                ...
                driver_obj.dnf = True if position["ReasonOutStr"] != "Running" else False
                ...
                driver_obj.playoff_points += 5 if race_manager.race_weekend.stage_results[2].stage_results.index(position) == 0 and driver_obj.points_eligible else 0
            else:

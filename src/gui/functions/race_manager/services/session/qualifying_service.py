...
class QualifyingService:
    ...
    def _set_field_size(cls, race_manager) -> None:
        for position in race_manager.ir["SessionInfo"]["Sessions"][race_manager.qualifying_session_num][
            "ResultsPositions"
        ]:
            if position["Position"] == 1:
                ...
                logging.info(f"Congrats to {race_manager.race_weekend.pole_winner} for winning the pole this week.")
            if position["Position"] > race_manager.race_settings.field_size:
                match = [
                    driver["CarNumber"]
                    for driver in race_manager.ir["DriverInfo"]["Drivers"]
                    if driver["CarIdx"] == position["CarIdx"] and "NODRIVER" not in driver["UserName"]
                ]
                if match:
                    logging.info(f"{match[0]} car missed the race")
                    race_manager.send_iracing_command(f"!dq {match[0]} #{match[0]} missed the race")
    ...
    def qualifying(cls, race_manager) -> None:
        ...
        logging.info(f"Setting field size to {race_manager.race_settings.field_size} cars")

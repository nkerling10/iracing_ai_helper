...
class Randomizer:
    ...
    def _assign_and_randomize_drivers(self) -> None:
        for car_entry in self.roster["drivers"]:
            ...
            if "NODRIVER" not in roster_driver.name:
                ...
                self._store_driver_attribute_compare(roster_driver_updated, orig_attrs)
    ...
    @staticmethod
    def _store_driver_attribute_compare(
        roster_driver_updated: object, orig_attrs: list
    ):
        print(roster_driver_updated.name)
        print(f"Skill: {orig_attrs[0]} -> {roster_driver_updated.driver_skill}")
        print(f"Smoothness: {orig_attrs[3]} -> {roster_driver_updated.smoothness}")

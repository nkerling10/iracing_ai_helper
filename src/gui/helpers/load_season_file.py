...
def _configure_season_file(config: object, season: dict):
    try:
        logger.info("Attempting to open season file")
    ...
    try:
        logger.info("Attempting to write updates to season file")
    ...
    if season.get("season_file") != "OFFICIAL":
        ...
        try:
            ...
            season_file["season_file"] = str(
                config.iracing_folder / "aiseasons" / f"{season.get("season_name")}.json"
            ).replace("\\", "/")
            season["season_file"] = str(
                config.iracing_folder / "aiseasons" / f"{season.get("season_name")}.json"
            ).replace("\\", "/")
            try:

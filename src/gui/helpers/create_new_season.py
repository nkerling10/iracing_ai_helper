...
def _create_local_season_file(values: dict, custom_tireset: int = 0) -> dict:
    ...
    season = {
                ...
                "tire_sets": "UNLIMITED" if values["__TIRESETSUNLIMITED__"] is True
                                else custom_tireset,
...
def _create_new_season(config) -> dict:
    layout = [
        ...
        [
            sg.Frame(layout=[[sg.InputText(key="__ROSTERNAME__")]],
                  title="Enter a name for the roster",
                  expand_x=True)
        ],
        [
            sg.Frame(layout=[[sg.Slider(range=(1, 100), default_value=100,
                                        orientation="horizontal", key="__FUELCAPACITY__",
                                        expand_x=True)]],
                    title="Select desired fuel capacity",
                    expand_x=True,
                    expand_y=True),
            sg.Frame(layout=[[sg.Checkbox("Custom", key="__TIRESETSCUSTOM__", expand_x=True,
                                          enable_events=True),
                              sg.Checkbox("Unlimited", key="__TIRESETSUNLIMITED__", expand_x=True, default=True,
                                          enable_events=True)],
